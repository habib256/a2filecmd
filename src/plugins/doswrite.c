...
static unsigned char vtoc[256], cat[256], ts[256], data[256], verify[512], seen[70];
static unsigned int allocated[260], needed, sectors, lists, length, address;
...
static unsigned char transfer(void) {
 unsigned int i,j,cur,left=length;unsigned char ok;
 source=RF(fopen)(a.full,"rb");if(!source)return 0;
 for(i=0;i<sectors;++i) {
 ...
 source=RF(fopen)(a.full,"rb");if(!source)return 0;left=length;
 for(i=0;i<sectors;++i) {
...
void __fastcall__ plugin_entry(const struct A2fcApi* api) {
 ...
 if(pan->fs || !a.full[0] || !a.selected->name[0] || other->fs!=FS_DOS33 || other->img_len) {
  note("Select a ProDOS file; open a real DOS 3.3 disk opposite.");return;
 }
 unit=(unsigned char)other->dir_key;
 n=unit_of(a.full,0);
 if((unit!=0x60 && unit!=0xE0) || !n || n==unit) {
  note("Use a separate Disk II in slot 6 for DOS 3.3.");return;
 }
 dw_mainbank();
 ...
 if(!source_length() || !audit()) {note("Copy refused: source, name collision or DOS disk consistency.");return;}
 sectors=((unsigned long)length+prefix+255)/256;
 lists=(sectors+121)/122;if(!lists)lists=1;
 needed=sectors+lists;n=0;
 for(i=48;i<560 && n<needed;++i)if(i/16!=17 && free_sector(i))allocated[n++]=i;
 if(n!=needed){note("DOS 3.3 disk full; nothing written.");return;}
 a.sprintf((char*)data,"Copy %s to DOS 3.3 S6,D%u?",a.selected->name,unit==0x60?1:2);
 ...
 note("Copied to DOS 3.3; source kept.");return;
refused:note("DOS 3.3 disk changed; nothing written.");return;
