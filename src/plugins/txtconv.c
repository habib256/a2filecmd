...
static void flush(void)
{
    if (olen) {
        if (checking) {
            if (T.fread(check, 1, olen, out) != olen || memcmp(check, obuf, olen)) fail = 1;
        } else if (T.fwrite(obuf, 1, olen, out) != olen) fail = 1;
    }
    nout += olen;
    olen = 0;
}
...
convert_pass:
    ...
    flush();
    /* A stale size can coincide with an interrupted read. Check the stream
     * error as well as the count before replacing either original. */
    if (ferror(in) || nin != e->size) { fail = 1; what = "Read"; }
    if (checking && !fail && T.fread(check, 1, 1, out)) fail = 1;
    if (ferror(out)) fail = 1;
    if (T.fclose(in)) fail = 1;
    if (T.fclose(out)) fail = 1;
    if (fail) goto errrm;
    if (!checking) {
        /* Re-run the deterministic conversion against the closed output.
         * Compare every generated byte and EOF before moving an original. */
        checking = 1; what = "Verify";
        in = T.fopen(T.full, "rb");
