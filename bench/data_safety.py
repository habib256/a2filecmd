...
A2FC_IMG=A2FILECMD-PRODOS-140K A2FC_PRESET=iie_unenh python3 bench/data_safety.py
"""
import os
import shutil
import subprocess
...
def main():
    with tempfile.TemporaryDirectory(prefix='a2fc-safety-') as tmp:
        ...
        with Pom2(hd,floppy2=po,port=6986+int(os.environ.get('A2FC_PORT_OFFSET','0'))) as p:
            ...
            s.select('DATA',0);s.key(b'C');s.wait(lambda:s.has('Overwrite/Skip/All/None?'),'overwrite confirmation')
            ...
            s.key(b'V');s.wait(lambda:s.has('Overwrite/Skip/All/None?'),'move overwrite confirmation')
