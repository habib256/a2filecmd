...
| `vdrive_printer.py` | VDrive ne touche jamais l'imprimante : une SSC imprimante en slot 1 (port 1 du //c) journalise chaque acces a ses registres (`pom2_playtest --printer-ssc LOG`, `Pom2(printer_log=...)`), sur le preset courant et le //c, seule puis a cote d'un VDrive en slot 2 (8 controles) |
...
| `hd.py` | le disque dur `.2mg` publie amorce-t-il, avec son dossier DEMO range par sorte de fichier (sans dossier CIDERPRESS, l'album dans PICTURES) ; une page brute s'affiche, un `.2MG` s'ouvre comme un dossier |
