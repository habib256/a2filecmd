# Documents

Les documents vivants décrivent l’état courant ; `history/` garde les
rapports datés, qui ne sont plus mis à jour.

| Document | Contenu | Langue |
| --- | --- | --- |
| [MANUAL.md](MANUAL.md) | Manuel utilisateur ; [A2FILECMD-MANUAL-EN.pdf](A2FILECMD-MANUAL-EN.pdf) en est l’édition imprimable ([MANUAL-PDF.md](MANUAL-PDF.md) explique sa régénération) | EN |
| [MINI-DOS33.md](MINI-DOS33.md) | A2FC Mini : l’édition Apple II+ 48 Ko DOS 3.3, ses touches, ses garanties d’écriture et ses mesures | EN |
| [DATA-SAFETY.md](DATA-SAFETY.md) | Revue des chemins d’écriture, de remplacement, d’effacement et d’usage d’AUX ; fichiers de récupération et limites | FR |
| [FILE-SERVICES.md](FILE-SERVICES.md) | Contrats des services de fichiers (réservation exclusive, installation récupérable, copie vérifiée) et journal du chantier 1 | FR |
| [MEMORY-BUDGETS.md](MEMORY-BUDGETS.md) | Réserves par zone mémoire, objectifs et journal de la consolidation | FR |
| [NIBCOPY.md](NIBCOPY.md) | Copie nibble de Disk II : ce qui est conservé, régénéré et vérifié | EN |
| [PURPLESOFT-FORMAT.md](PURPLESOFT-FORMAT.md) | Format des images Purplesoft `.FOTO1`/`.FOTO2` | FR |
| [SAMPLE-MEDIA.md](SAMPLE-MEDIA.md) | Prise en charge du corpus `SAMPLE.MEDIA` (polices MGTK, LZ4FH, Print Shop, Integer BASIC) | FR |
| [screenshots/](screenshots/) | Captures des deux panneaux | — |

## history/

| Rapport | Objet |
| --- | --- |
| [RELEASE-0.8.6.md](history/RELEASE-0.8.6.md) | Qualification de la 0.8.6, la release qui clôt la partie Commander, avec des comparaisons d’octets réelles sur le disque dur émulé |
| [RELEASE-0.8.5.md](history/RELEASE-0.8.5.md) | Qualification locale et CI de la 0.8.5 ; première release où la Mini porte le numéro de l’édition ProDOS |
| [BUG-HUNT-0.8.0.md](history/BUG-HUNT-0.8.0.md) | Dernière passe avant la 0.8.0 : 219 contrôles POM2, mutations, empreintes |
| [BUG-HUNT-0.7.6.md](history/BUG-HUNT-0.7.6.md) | Revue avant la 0.7.6 : dépassement de pile, catalogues cycliques, curseur invalide |
| [STABILIZATION.md](history/STABILIZATION.md) | Suivi des quatre étapes de l’évaluation du 11 septembre 2026, jusqu’au candidat 0.8.0 |
| [PT3-CREDITS-PROPOSAL.md](history/PT3-CREDITS-PROPOSAL.md) | Titre et crédits PT3, proposition implémentée |
| [ANNONCE.md](history/ANNONCE.md) | Brouillon d’annonce pour les groupes Apple II, écrit pour la 0.5 |

La feuille de route est [TODO.md](../TODO.md) ; les versions publiées sont
dans le [CHANGELOG](../CHANGELOG.md).
