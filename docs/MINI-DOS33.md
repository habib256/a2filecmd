...
The disk image `dist/A2FC-MINI-DOS33-0.6.0.dsk` boots through the Applesoft
...
| [ / ] | First / last file |
| Return, or 2 | Preview according to file type |
| T / H | Text / hexadecimal preview; also available inside the preview |
| Escape in preview or help | Return to the panels |
...
`/` replaces D for drive selection, and Ctrl-R replaces R for rereading.
D and R remain reserved for the ProDOS Delete/Rename conventions; Mini does
not implement those operations. Escape at the browser does nothing because
...
or deleted. This edition creates files by copying; it does not move, replace,
rename, delete, edit, or format.
...
The copy engine reads a batch of sixteen sectors from the source, then
writes and verifies that batch on the target, then re-reads both disks
for the final comparison. It performs the same six disk operations per
sector as before and in the same order of safety; what changed is that
the drives alternate four times per batch instead of four times per
sector, and a change of drive costs a seek and a motor spin-up.
...
The raw binary is `build-mini/A2FC.MINI`. The disk builder adds its `$2000` load
address and length. `make mini` is independent of the ProDOS editions and of
cc65's C compiler and runtime: eight assembly modules, linked by
...
- `$0800–$1FFF`: reserved for the Applesoft launcher.
- `$2000–$7FFF`: code, read-only data, then BSS, bounded together by ld65.
- No software stack: page one is the only one.
- This build ends at `$7BB8`, leaving 1 096 bytes below DOS. `make mini` prints
  the figure at every link and `tools/check_mini_layout.py` refuses a build
  that would reach into DOS's buffers.
- The binary is 7 750 bytes and BSS 15 730, of which 4 096 are the copy batch.
...
destinations; empty files; and files needing up to five T/S lists. What is
asserted is the bytes: the source unchanged, every pre-existing target byte
...
long names, disk errors, English text, and physical screen writes through memory
watchpoints: unchanged characters must never be rewritten. The second checks
cancellation, ignored confirmation keys, physical protection, verified copying,
and collisions. It then reloads the copied binary with **DOS BLOAD** and creates
another file with **DOS SAVE**. Contents and allocations are independently
checked afterward. The third reports the cycle costs in the table above.
