...
- **One extent, so 16 KB at most.** A longer file needs several entries, and a cut between two of them gives a file whose length comes from an extent that is there while an earlier one is not. Sixteen kilobytes is an eighth of a 5.25" CP/M disk; a bigger file is skipped rather than written in a way that could read as something else. A name that cannot be an eight-and-three is skipped too, not mangled.
...
- Margin left in the window: 364 bytes on 6502, 347 on 65C02.
