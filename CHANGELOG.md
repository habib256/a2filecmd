...
- Prepare the 0.8.5 release. A2FC Mini now carries the same release number as the ProDOS edition, printed from the one `A2FC_VERSION` line of the Makefile (splash, HELLO, disk name and benches), and its `A2FC-MINI-DOS33-0.8.5.dsk` is listed with the release files. README, manual and bench documentation use the new names.

- Fix DOS extraction metadata and EOF: BIN preserves its DOS load address, BIN/BAS/INT exclude sector padding. DOSGET moves to FILES/XL with protected entry/sector scratch, exclusive output ownership, cycle/bounds checks, cancellation and checked I/O/cleanup. Return recognizes legacy BIN `M.*` DUET candidates. IDENT adds DUET/PT3, font and specialized picture families, honors packed attributes before raw sizes, and reports read/seek/close errors. FIXTYPES validates complete DUET candidates (including legacy padding), proposes `$D5/$D0E7` without renaming, and confirms every attribute change before writing. Metadata is reread around confirmation; access/dates and file contents are preserved. API v5 adds the active-entry snapshot without changing service offsets.

- UNSHRINK now owns each exclusive output reservation through failed closes and reports a failed cleanup by naming the retained file; retries preserve its bytes. Exact reads replace unchecked seeks through skipped threads and padding, stream errors and archive-close failures prevent success, and Escape cancels between blocks. Short attribute headers, inconsistent stored lengths and invalid decoder consumption are refused; unsupported compression remains in the final report. New C fault-injection tests check preserved bytes, including combined close/cleanup failures, and the native ShrinkIt bench also checks the stack guard. Complete NuFX CRC validation and output readback remain open.

- Browsing lo-res and double lo-res pictures with Left/Right keeps the current picture on the air, in graphics mode, until its neighbour is drawn over it: the text page is no longer cleared for a loading message, and the panels are not redrawn between two pictures (for hi-res pictures, the loading screen now stays until the next picture instead of flashing the panels).

- The `!` menu's category page now shows, for each category, how many overlays it holds and what they are for, on the same two-column rows as the tool lists; a category's list repeats that row above it. Routing overlays are hidden through one list instead of seven comparisons, so MENU still fits the floppy budget (3,582 bytes on 6502, two under its seven data blocks).

- Play Electric Duet songs with the new DUET overlay (MEDIA/XL): Return on a file of Apple II DeskTop's type `$D5`/`$D0E7` or named `.ED`, or DUET from the `!` menu on any file. The song is read, closed and validated in main RAM (complete records, terminator, a note, 7,168 bytes at most) before playback; a Mockingboard plays it when present, otherwise the speaker does through Alex Patalenski's improved player, transcribed cycle for cycle into an aligned segment; **1**/**2** switch outputs, **D** cycles the speaker pulse width (1/8 by default, 1/4, Patalenski's 1/16), **P** pauses, **Escape** returns, Left/Right browse songs. The AY mixer is set to tones only, so the card plays without noise. The demo disk gains `CANON.ED`. Host tests cover validation, I/O faults and both outputs; POM2 benches cover both CPUs, both outputs, pause, tracks, natural end, A2DeskTop's `JESU.JOY`, bad data, AUX and the stack floor. Found on the way: cc65 2.19 indexes a page-aligned constant address through `ptr1` without writing its low byte.

- A2FC Mini DOS 3.3 file-system bug hunt. Delete, lock and rename no longer identify a file by its sanitised panel name alone: the on-disk entry must carry the T/S pointer, type and sector count the panel showed, else `DISK CHANGED`; a name whose unprintable bytes showed as `?` could delete another file whose real name had a `?` there, and a sibling disk with the same names could lose the wrong sectors. Copy and create re-read the destination VTOC after the prompt and refuse a swapped disk instead of writing the stale reservation image over it. A catalog chain that loops (a corrupt link met after the panel read) is refused rather than followed forever with the drive on, in copy's destination scan and in delete/lock/rename. The DOS UNDELETE mark now keeps the T/S list track in the last name byte (entry `$20`) as DOS does, not the first. A write-protected disk is reported as such by delete, lock and rename before any write, without latching `del_fault`. `load_file` reports when a file went past the 8 KB area so E refuses a text file whose catalog count understated its size instead of saving a truncated copy. Eleven new sim65 regressions; the harness now turns a looping 6502 routine into a test failure. The DELETE, LOCK and UNLOCK footer prompts show the file name again instead of its first letter followed by blanks (`put` rebuilds `ptr`; the name is now read through `ptr2`). The two POM2 benches follow the 0.7 footer prompts, wait for the panel reread after each result, and `bench/mini33_write.py` swaps the target disk at the COPY prompt and checks it comes back untouched. Every write operation now goes back to the catalog slot the panel read (`ent_slot`), not to a name: copy takes the raw 35-byte entry from the source so catalog-art names travel byte for byte and collide as DOS sees them, and copy re-reads that slot after Y so a swapped or deleted source is refused as `DISK CHANGED` instead of being read sector by sector. Disks formatted without DOS are supported: tracks 1–2 are accepted for file data, and used for new files, when the VTOC frees some of their sectors; an ordinary disk, with those tracks fully allocated by DOS, still refuses a chain into them. Rename's collision scan reports a read error or a broken chain as such instead of `NAME EXISTS`; E tells an unreadable text file from an invalid T/S list. The copy engine's prepare-time routines moved below the working area to keep 268 bytes free under DOS.
