...
### A hunt through the writing, and what it turned up

- **A bitmap that lies no longer costs the volume.** Given a ProDOS image whose bitmap marks the volume directory and the bitmap itself as free -- a damaged image, which is exactly what FIXIT exists for -- IMGPUT wrote the file over blocks 1 to 6 and the volume did not read back at all. ProDOS trusts the bitmap and would do the same; that is not a reason to. The allocator now starts past the bitmap's last page and never below block 6, so the boot blocks, the four of the volume directory and the bitmap are out of reach whatever the bitmap says. `tools/test_imgput.py` marks them free four different ways and checks the file lands above them and the volume still reads.
- The first attempt at that fix silently did nothing: the patch did not match the text it meant to replace, `floor_block` stayed zero, and the guard compiled into `scan < 0 ? ... : scan`. The hunt caught it because it was re-run against the same lie afterwards, not because the tests went green -- they did, both before and after.
- **The containers nobody had written to.** A `.2MG` puts a header in front of the volume and a `.DSK` stores the halves of every block in DOS sector order; every test and every bench used a flat `.PO`. All three writers handle both correctly -- checked, not assumed -- and the three test files now say so. It matters most for Pascal: the Asimov disks that PASCAL reads are `.do` files, so the DOS order is the real case there, not the exotic one.
- Two things the hunt cleared rather than fixed. Copying a mounted image into itself cannot happen: a file of N blocks never fits in a volume of N blocks, so the arithmetic refuses it before any guard would. And the bitmap page cache is now invalidated where its buffer is taken over for the directory block -- not a defect today, because nothing reads it afterwards, but the next edit would have inherited it.
