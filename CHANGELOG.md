...
- The name no longer ends in a slash on that line: fifteen characters and a slash left no room for the two marks, and `<DIR>` says the same thing. `tools/test_display.py` renders the real `draw_entry` and compares the four lines (tagged or not, locked or not, up to a fifteen-character name) character by character. The resident gained 27 bytes on the way: the two lines now share their head, and the `sprintf` that built the name with its slash is gone.
