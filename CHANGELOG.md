...
- MOVE from the `!` menu on the selected directory, no mark, bound for another volume: the core walks the tree as V does instead of pointing at V. A recount of the destination before deleting a moved directory's source was tried and withdrawn: copy_one only returns success after walking the tree to its end and reading back every file it wrote, and the recount cost 85 resident bytes for no further guarantee. A moved directory's summary now counts the files copied, not the files and directories the delete walk visited ("2 files moved", not "6"). 65C02: 299 bytes free.

- VOLNAME moves from BOOT to the DISKTOOLS floppy, beside VOLINFO; XL keeps every tool. BOOT gains four blocks of room after weeks at zero, so a fix in RUN, MENU, BATCH or the launcher no longer has to shave bytes to fit. `bench/goto.py` follows the recoverable installation: after a failed rename the kept GOTO.TMP blocks saving until it is reviewed and removed, and the bench does exactly that between its two injected failures.

- The confirmation before launching a program spells out the way back, for example `Run HELLO? Back: -/A2FC6502/A2FILE.SYSTEM`: BASIC.SYSTEM keeps its prefix on the launched program's directory, where its data files live, so the bare `-A2FILE.SYSTEM` the demo used to advertise only resolved from A2FC's own directory (`PATH NOT FOUND` elsewhere). The demo's HELLO now says `BYE, THEN PICK A2FILE.SYSTEM TO RETURN`, which the ProDOS 2.4.3 selector honours, and the manual describes both routes. The session bench types the command it reads from the confirmation and requires A2FC back on its panels; the scenario is no longer isolated. To keep BOOT within 280 blocks, the floppy launcher's title page drops one descriptive line, abbreviates month names and two notes (6,134 bytes, under its 12 blocks).

- Marked directories move. Space and Ctrl-T now tag directories (`..` never), and the marked MOVE accepts them: on the same volume the directory entry is rewritten without copying, as for a file; bound for another volume, a directory gets what V does -- counted, copied and read back file by file, its source deleted only once every file has arrived; a stopped copy or a skipped file keeps the whole source and stops the batch with its reason. The confirmation counts directories among the entries to move. C, V and D already walked tagged directories; CRC, VERIFY and FIXTYPES skip them, DOSWRITE refuses them by type, and TAGPAT still tags files only. MOVE, which knows the ProDOS units, hands such a directory back to the resident with a marked note instead of refusing it. Resident cost 255 bytes (65C02: 335 free); a BATCH phase rebuilds the batch paths after a tree move, since the copy engine shares their storage; BOOT stays within its 280 blocks, one MENU message shortened for the block BATCH grew by. Host tests cover a directory record through the manifest and the cross-volume flag; `bench/roi.py` moves a marked directory with two levels on the same volume and to another disk and reads the trees back.

- The tree walks behind copying, moving and deleting a directory no longer recurse: one resident engine (`src/tree_walk.h`) visits the tree with one three-byte frame per level in fixed arrays, the entries of each level stacked in the same pool as before. A 20-level tree is now deleted, and copied as deep as the 64-character ProDOS path allows for the destination and its recoverable-copy temporary name (18 levels from the bench's destination); beyond that the copy stops cleanly with the source kept. The refusals stay explicit and happen before any write: a path beyond 64 characters, more than 213 entries along one path, or an unreadable directory. Deleting still walks the whole tree first. The DELETE overlay keeps only the call and gains 405 bytes; the resident spends 257 bytes of code and 99 of BSS (65C02: 590 bytes free). Host test `tools/test_tree_walk.py` checks order, bounds and error stops on a fake file system; `bench/tree_safety.py` now expects an 18-level copy to complete, a 20-level copy to stop on the path limit and its tree to be deleted, and a 221-entry path to be refused with every disk byte intact.

- The DOS 3.3 catalog reader and the directory walk of a disk image opened as a folder move out of the resident program into a CATALOG overlay (BOOT and XL), loaded when a DOS 3.3 disk, a DOS-order image or a ProDOS image is read as a panel. A read requested while another overlay's code is still running (help, the sort hosted by TEXT, a plugin refreshing the panel it wrote to) is deferred: the panel is left empty and marked, and the main loop rereads it before the next key, so the running overlay is never overwritten. A directory change that NAV asks for while the read is deferred lands on the wanted entry once the panel is settled. Resident headroom on the 65C02 build rises from 23 to 847 bytes (6502: 518 to 1,300); ceilings, C stack and BSS ceilings are unchanged. Host test and POM2 bench (`bench/catalog_overlay.py`) cover deferred and direct reads of both kinds on both CPUs.
