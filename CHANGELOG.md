...
- Changed the release to two editions from one tree. The **floppy edition**, `A2FILECMD.po` and `.dsk`, is now the 6502 build with the file manager and the disk tools only (`HELP`, `TEXT`, `HEX`, `DELETE`, `RUN`, `ATTR`, `MENU`, `DISKIMG`, `IMGFS`, `DOS33`, the formatter; no BASIC.SYSTEM), so it runs on any Apple II with 128 KB and 80 columns, the 1983 IIe included, and keeps 77 blocks free for the disk tools to come. The **complete edition**, `A2FILECMD.2mg`, is the 65C02 build with every overlay, the mouse, BASIC.SYSTEM, `DEMO/` and `IMGHGR/`. `make disk` builds both; the `-6502` images are gone. The title page names the edition. On the floppy, a key whose overlay is absent says so.
