"""Execute overlay_run: target text precedes cleanup and panel restoration."""
...
static void entry(const struct A2fcApi* a){
 ...
 if(media_key(key) && key!=KEY_ESC){
  if(!routed || strncmp(output,"Loading ",8) || strcmp(output+8,target))abort();
 }else if(routed || strcmp(output,"OLD FILE"))abort();
...
int main(int argc,char**argv){
 ...
 total=(mode==0 || mode==7)?3:300;
 for(i=0;i<total;++i)sprintf(all[i].name,"F%03u.PT3",i);
 strcpy(all[0].name,"A.MB");strcpy(all[total-1].name,"B.MB");
 target=mode==6?"A.MB":"B.MB";
 ...
 else if(calls!=2 || draws!=(mode==7?1:2) || switches!=(mode==7?1:2))return 3;
 return 0;
}
'''
