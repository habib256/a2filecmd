...
class SplitLoadLayout(unittest.TestCase):
    def setUp(self):
        ...
        self.overlays = dict(IMAGE=0x1F60 - 0x1B00, HELP=0x1EE0 - 0x1B00, TEXT=0x280,
                             ...
                             EDIT=0xC00, MENU=0x600, DISKIMG=0x900, IMGFS=0x300, DOS33=0x400, UNSHRINK=0x600, BASLIST=0x340)
        for name in ('MUSIC', 'RUN', 'ATTR', 'EDIT', 'MENU', 'DISKIMG', 'IMGFS', 'DOS33', 'UNSHRINK', 'BASLIST'):
            self.s['__%s_START__' % name] = 0x1B00
            self.s['__%s_LAST__' % name] = 0x1B00 + self.overlays[name]
        for name in ('IMAGE', 'TEXT', 'HEX', 'DELETE', 'HELP', 'MUSIC', 'RUN', 'ATTR', 'EDIT', 'MENU', 'DISKIMG', 'IMGFS', 'DOS33', 'UNSHRINK', 'BASLIST'):
