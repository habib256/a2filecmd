...
class SplitLoadLayout(unittest.TestCase):
    def setUp(self):
        ...
        self.overlays = dict(NAV=0x400, DOS33=0x300, BATCH=0x900, FORMAT=0x1800, IMAGE=0x1F60 - 0x1B00, HELP=0x1EE0 - 0x1B00, TEXT=0x280,
        ...
        for name in ('NAV', 'DOS33', 'BATCH', 'OPEN', 'COPY', 'FORMAT', 'RUN', 'ATTR', 'EDIT', 'MENU', 'DISKIMG', 'IMGFS', 'DOSGET', 'UNSHRINK', 'BASLIST', 'COMPARE', 'SEARCH', 'BINARY2', 'AWP'):
            self.s['__%s_START__' % name] = 0x1B00
            self.s['__%s_LAST__' % name] = 0x1B00 + self.overlays[name]
        for name in ('NAV', 'DOS33', 'BATCH', 'OPEN', 'COPY', 'FORMAT', 'IMAGE', 'TEXT', 'HEX', 'DELETE', 'HELP', 'RUN', 'ATTR', 'EDIT', 'MENU', 'DISKIMG', 'IMGFS', 'DOSGET', 'UNSHRINK', 'BASLIST', 'COMPARE', 'SEARCH', 'BINARY2', 'AWP'):
