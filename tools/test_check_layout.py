...
class SplitLoadLayout(unittest.TestCase):
    def setUp(self):
        ...
        for name in ('NAV', 'BATCH', 'OPEN', 'COPY', 'FORMAT', 'RUN', 'ATTR', 'EDIT', 'MENU', 'DISKIMG', 'IMGFS', 'DOSGET', 'UNSHRINK', 'BASLIST', 'COMPARE', 'SEARCH', 'BINARY2', 'AWP'):
            self.s['__%s_START__' % name] = 0x1B00
            self.s['__%s_LAST__' % name] = 0x1B00 + self.overlays[name]
        for name in ('NAV', 'BATCH', 'OPEN', 'COPY', 'FORMAT', 'IMAGE', 'TEXT', 'HEX', 'DELETE', 'HELP', 'RUN', 'ATTR', 'EDIT', 'MENU', 'DISKIMG', 'IMGFS', 'DOSGET', 'UNSHRINK', 'BASLIST', 'COMPARE', 'SEARCH', 'BINARY2', 'AWP'):
