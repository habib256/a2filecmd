...
class SplitLoadLayout(unittest.TestCase):
    def setUp(self):
        ...
        self.overlays = dict(NAV=0x400, CATALOG=0x300, BATCH=0x900, FORMAT=0x1800, IMAGE=0x1F60 - 0x1B00, HELP=0x1EE0 - 0x1B00, TEXT=0x280,
                             HEX=0x2F0, DELETE=0x300, RUN=0x200, ATTR=0x300,
                             OPEN=0x200, COPY=0x300, EDIT=0xC00, MENU=0x600, DISKIMG=0x900, IMGFS=0x300, DOSGET=0x400, UNSHRINK=0x600, BASLIST=0x340, COMPARE=0x200, SEARCH=0x200, BINARY2=0x300, AWP=0x300)
        for name in ('NAV', 'CATALOG', 'BATCH', 'OPEN', 'COPY', 'FORMAT', 'RUN', 'ATTR', 'EDIT', 'MENU', 'DISKIMG', 'IMGFS', 'DOSGET', 'UNSHRINK', 'BASLIST', 'COMPARE', 'SEARCH', 'BINARY2', 'AWP'):
            self.s['__%s_START__' % name] = 0x1B00
            self.s['__%s_LAST__' % name] = 0x1B00 + self.overlays[name]
        for name in ('NAV', 'CATALOG', 'BATCH', 'OPEN', 'COPY', 'FORMAT', 'IMAGE', 'TEXT', 'HEX', 'DELETE', 'HELP', 'RUN', 'ATTR', 'EDIT', 'MENU', 'DISKIMG', 'IMGFS', 'DOSGET', 'UNSHRINK', 'BASLIST', 'COMPARE', 'SEARCH', 'BINARY2', 'AWP'):
