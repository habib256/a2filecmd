...
static int ferr(FILE* f){return fault==1 || (fault==7&&phase) || ferror(f);}
static int cls(FILE* f){int r=fclose(f);++closed;return fault==2?EOF:r;}
static int seekf(FILE*f,long n,int how){++misses;return fault==5?-1:fseek(f,n,how);}
static FILE* opn(const char* p,const char* m){FILE*f=fault==3?NULL:fopen(p,m);if(f)++opened;return f;}
unsigned char host_song[512],host_cache[11][256];
#define ferror ferr
#include "src/plugins/pt3.c"
#undef ferror
unsigned int pt_end;
void __fastcall__ pt_tables(unsigned char* p){memset(p,0xA5,512);}
...
unsigned char pt_frame(void){
 ...
 for(page=2;page*256<n;++page){
  slot=pt_page(page);if(!slot)return 1;slot-=CACHE_PAGE;
 ...
 if(pt_pages[0]!=HEADER_PAGE || pt_pages[1]!=HEADER_PAGE+1)abort();
 return 2;
}
...
static void info(const unsigned char* h){if(memcmp(h,original,n<512?n:512))abort();}
...
int main(int argc,char**argv){
 ...
 if(started){for(i=0;i<512;++i)if(buf[i]!=0xA5)abort();if(strcmp(sel,"TEST.PT3"))abort();}
