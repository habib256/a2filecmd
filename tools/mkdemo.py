...
decodage des couleurs est juste.
  ...
  CANON.ED   un canon a deux voix pour le haut-parleur, format Electric Duet
...
# ── Electric Duet ──────────────────────────────────────────────────────────
def ed_pitch(midi):
    """The Electric Duet pitch byte of a MIDI note: the speaker player counts
    that many 69-cycle iterations per period (1.0227 MHz / 69 = 14,822 Hz)."""
    return round(14822 / (440 * 2 ** ((midi - 69) / 12)))


def duet():
    """Frere Jacques as a two-voice canon, the second voice two bars behind.
    Three-byte records: a duration in units of 256 iterations (17.3 ms),
    then the two pitches (0 = rest); a leading 1 sets the duty-cycle shifts,
    a leading 0 ends the song. Notes are articulated by a short rest."""
    G3, C4, D4, E4, F4, G4, A4 = 55, 60, 62, 64, 65, 67, 69
    melody = [(C4, 2), (D4, 2), (E4, 2), (C4, 2)] * 2 + [(E4, 2), (F4, 2), (G4, 4)] * 2 \
        + [(G4, 1), (A4, 1), (G4, 1), (F4, 1), (E4, 2), (C4, 2)] * 2 + [(C4, 2), (G3, 2), (C4, 4)] * 2
    ticks = 6                                   # per eighth note, 2 units each
    def voice(delay):
        out = [0] * (delay * ticks)
        for midi, eighths in melody:
            n = eighths * ticks
            out += [ed_pitch(midi)] * (n - 1) + [0]
        return out
    v1, v2 = voice(0), voice(16)
    v1 += [0] * (len(v2) - len(v1))
    s = bytearray((1, 2, 3))
    for p1, p2 in zip(v1, v2):
        if len(s) > 3 and s[-2] == p1 and s[-1] == p2 and s[-3] < 254:
            s[-3] += 2
        else:
            s += bytes((2, p1, p2))
    return bytes(s) + bytes(3)
...
CANON.ED is Frere Jacques as a two-voice Electric Duet canon,
played on the speaker, or on a Mockingboard when there is one.
...
def make(out, full=True):
    ...
    (out / 'CANON.ED#D5D0E7').write_bytes(duet())
